...
`,s=e.user_input||"";console.log(`Problem agent analyzing business idea (length: ${s.length}): "${s.substring(0,50)}..."`);let r=0,a=null;for(;r<=2;)try{r>0&&console.log(`Retrying Problem Agent analysis (attempt ${r}/2)`);let e=(await m.chat.completions.create({model:"gpt-3.5-turbo-1106",messages:[{role:"system",content:"You are the Problem Specialist Agent (The Diagnostician). Your role is to extract, clarify and strengthen the actual problem statement from the business idea."},{role:"user",content:t}],temperature:.5,response_format:{type:"json_object"}})).choices[0].message.content;if(!e)throw Error("No response from Problem Agent");try{let t=JSON.parse(e);return t.improved_problem_statement||(t.improved_problem_statement=s.substring(0,200)),t.severity_index&&"number"==typeof t.severity_index||(t.severity_index=5),t.problem_framing||(t.problem_framing="niche"),t.root_causes&&Array.isArray(t.root_causes)||(t.root_causes=["Unable to determine root causes"]),t.score&&"number"==typeof t.score||(t.score=70),t.reasoning||(t.reasoning="Analysis completed with default values"),t}catch(s){let t=s instanceof Error?s.message:"Unknown parsing error";throw console.error("Problem Agent returned invalid JSON:",t),console.error("Response content:",e.substring(0,500)),Error(`Problem Agent returned invalid JSON: ${t}`)}}catch(e){if(console.error(`Problem Agent analysis failed (attempt ${r+1}/3):`,(e instanceof Error?e:Error(String(e))).message),++r<=2){let e=Math.min(1e3*Math.pow(2,r-1),8e3);console.log(`Waiting ${e}ms before retry...`),await new Promise(t=>setTimeout(t,e))}}return console.log("All Problem Agent retries failed, generating fallback analysis"),{improved_problem_statement:s.substring(0,500),severity_index:5,problem_framing:"niche",root_causes:["Unable to determine due to processing issues"],score:60,reasoning:"Auto-generated due to API processing issues"}}async function f(e){let t=`
...
`;e.user_input,console.log(`Market agent analyzing business context (enhanced problem: ${e.enhanced_problem_statement?.substring(0,50)||"None"}...)`);let s=0,r=null;for(;s<=2;)try{s>0&&console.log(`Retrying Market Agent analysis (attempt ${s}/2)`);let e=(await m.chat.completions.create({model:0===s?"gpt-4-turbo-preview":"gpt-3.5-turbo-1106",messages:[{role:"system",content:"You are the Market Specialist Agent (The Opportunity Validator). Your role is to analyze market size, growth potential, and validate the market opportunity."},{role:"user",content:t}],temperature:.5,response_format:{type:"json_object"}})).choices[0].message.content;if(!e)throw Error("No response from Market Agent");try{let t=JSON.parse(e);return("number"!=typeof t.tam||isNaN(t.tam))&&(t.tam=1e9),("number"!=typeof t.sam||isNaN(t.sam))&&(t.sam=.3*t.tam),("number"!=typeof t.som||isNaN(t.som))&&(t.som=.1*t.sam),t.growth_rate||(t.growth_rate="10-15% annually"),t.market_demand||(t.market_demand="Moderate demand with growing interest"),t.why_now||(t.why_now="Current market conditions are favorable for this solution"),t.score&&"number"==typeof t.score||(t.score=65),t.reasoning||(t.reasoning="Analysis completed with default values"),t}catch(s){let t=s instanceof Error?s.message:"Unknown parsing error";throw console.error("Market Agent returned invalid JSON:",t),console.error("Response content:",e.substring(0,500)),Error(`Market Agent returned invalid JSON: ${t}`)}}catch(e){if(console.error(`Market Agent analysis failed (attempt ${s+1}/3):`,(e instanceof Error?e:Error(String(e))).message),++s<=2){let e=Math.min(1e3*Math.pow(2,s-1),8e3);console.log(`Waiting ${e}ms before retry...`),await new Promise(t=>setTimeout(t,e))}}return console.log("All Market Agent retries failed, generating fallback analysis"),{tam:1e9,sam:3e8,som:3e7,growth_rate:"Unknown, estimated 10-15%",market_demand:"Unable to determine precisely",why_now:"Current market conditions may be suitable",score:60,reasoning:"Auto-generated due to API processing issues"}}async function h(e){let t=`
