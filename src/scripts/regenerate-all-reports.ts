#!/usr/bin/env ts-node
/**
 * Script to regenerate report_data for ALL records in the validation_analyses table
 * This will use the new schema format and ensure consistency
 * 
 * Run with: npx ts-node src/scripts/regenerate-all-reports.ts
 */
...
async function regenerateAllReports() {
  console.log('Starting regeneration of report_data for ALL analyses...')
  ...
  console.log('All report data regenerated successfully!')
}

// Function to generate enhanced report data from a validation
function generateEnhancedReportData(validation: ValidationWithAnalysis): ReportData {
  ...
  // Generate a more intelligent AI interpretation without excessive truncation
  const businessIdea = validation.business_idea || '';
  const businessType = validation.business_type || "SaaS";
  const targetAudience = validation.target_audience || "customers";
  
  // Extract key details for the AI interpretation
  const firstSentence = businessIdea.split('.')[0];
  const shortenedIdea = firstSentence.length > 150 
    ? firstSentence.substring(0, 150) + '...' 
    : firstSentence;
  
  // Create a more comprehensive AI interpretation
  const aiInterpretation = `Based on your description, you're building a ${businessType} solution that addresses ${targetAudience} needs through ${shortenedIdea}. This analysis evaluates market opportunity, business viability, and investor potential.`;
  ...
  return {
    ...
    user_input: validation.business_idea, // Preserve full business idea without truncation
    ...
    problem: {
      summary: (marketAnalysis as any).target_audience || "Customer needs identified in the business concept",
      ...
      ai_summary: (marketAnalysis as any).target_audience || 
    ...
    target_audience: {
      ...
      motivation: `These segments are motivated by the need to ${validation.business_idea.substring(0, 30)}... more effectively and efficiently.`,
      buyer_user_table: {
    ...
    market: {
      ...
      som,
      trends: [
        (marketAnalysis as any).growth_potential || "Growing market demand", 
